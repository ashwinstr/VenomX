...
@venom.trigger(cmd="download")
async def down_load(_, message: MyMessage):
    """
    CMD: DOWNLOAD
    ...
    if (not message.replied or not message.replied.media) and not message.input_str:
        await response.edit(
...
async def telegram_download(
    message: MyMessage, response: MyMessage, path: str, file_name: str | None = None
) -> DownloadedFile:
    """
