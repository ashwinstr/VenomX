...
async def photo_upload(
    file: DownloadedFile, has_spoiler: bool
) -> dict[str, venom.send_photo, dict]:
    return dict(
        method=venom.send_photo,
...
async def audio_upload(
    file: DownloadedFile, has_spoiler: bool
) -> dict[str, venom.send_audio, dict]:
    return dict(
        method=venom.send_audio,
        kwargs=dict(
...
async def doc_upload(
    file: DownloadedFile, has_spoiler: bool
) -> dict[str, venom.send_document, dict]:
    return dict(
        method=venom.send_document,
...
@venom.trigger(cmd="upload")
async def upload(_, message: MyMessage):
    """
    CMD: UPLOAD
    ...
    if input.startswith("http") and not file_check(input):
        ...
        if not venom.me.is_premium and dl_obj.size > 1999:
            await response.edit(
